...
export class CouncilBrain {
    ...
    public assessHealth(toolHistory: string[], errorCount: number): CouncilStrategy {
        ...
        if (lastYolo && lastYolo.confidence >= 2.5) {
            ...
            if (lastYolo.strategy === 'HYPE' || lastYolo.strategy === 'EXECUTE') {
                ...
                if (errorCount < 3) return lastYolo.strategy;
            }
        }

        if (errorCount >= 4) return 'PANIC';
        if (errorCount >= 2) return 'DEBUG';

        if (toolHistory.length >= 3) {
            const last3 = toolHistory.slice(-3);
            if (last3[0] === last3[1] && last3[1] === last3[2]) return 'RESEARCH';
        }

        if (toolHistory.length >= 4) {
            const last4 = toolHistory.slice(-4);
            if (last4[0] === last4[2] && last4[1] === last4[3]) return 'RESEARCH';
        }

        if (toolHistory.length >= 6) {
            const last6 = toolHistory.slice(-6);
            if (new Set(last6).size < 3) return 'RESEARCH';
        }

        if (this.assessMomentum() === 'HYPE') return 'HYPE';

        return 'EXECUTE';
    }

    public assessMomentum(): CouncilStrategy | null {
    ...
    public predictFailure(strategy: string, filePath?: string): string | null {
    ...
    public detectStrategicLoop(history: { strategy: CouncilStrategy }[]): boolean {
    ...
    public classifyError(message: string): ErrorCategory {
        ...
        if (lower.includes('syntax') || lower.includes('unexpected token') || lower.includes('parse error')) return 'SYNTAX';
        ...
        if (lower.includes('type') || lower.includes('undefined') || lower.includes('null')) return 'LOGIC';
        return 'UNKNOWN';
    }

    public detectTunnelVision(pendingObjectiveCount: number): boolean {
    ...
    public calculateSessionScore(): { score: number, grade: string } {
